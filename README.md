...
Si CMake no encuentra Qt automáticamente, especifica `CMAKE_PREFIX_PATH` (ejemplo MSVC 6.9.2):
```bash
cmake -S . -B build -DCMAKE_PREFIX_PATH="C:/Qt_v6_9/6.9.2/msvc2022_64"
cmake --build build --config Release
```

También puedes usar MinGW (si instalaste el kit MinGW de Qt 6.9.2):
```bash
cmake -S . -B build-mingw -G "MinGW Makefiles" -DCMAKE_PREFIX_PATH="C:/Qt_v6_9/6.9.2/mingw_64"
...
En MinGW, usa `windeployqt` para copiar las dependencias Qt junto al .exe (6.9.2):
```powershell
C:/Qt_v6_9/6.9.2/mingw_64/bin/windeployqt.exe --release build-mingw/money_home_gui.exe
```
...
- Qt 6.9.2 for Android (arm64-v8a), Widgets y Charts
- SDK Platform 34, Build-Tools 34.0.0, NDK r25c (25.2.9519653), JDK 17

Cómo compilar en Qt Creator:
- Instala con el Qt Installer/Qt Android Tools: Android SDK/NDK y un kit Android (p. ej. Qt 6.9.2 for Android)
...
- Host (Windows): `C:\Qt_v6_9\6.9.2\msvc2022_64`
- Android (arm64): `C:\Qt_v6_9\6.9.2\android_arm64_v8a`
- SDK: `C:\Users\TU_USUARIO\AppData\Local\Android\Sdk`
- NDK: `C:\Users\TU_USUARIO\AppData\Local\Android\Sdk\ndk\25.2.9519653`

Importante: la NDK suele estar dentro del Android SDK, no en las carpetas de Qt Creator (evita rutas como `...\QtCreator\...\modules\Android\ndk`). Verifica la ruta exacta en Qt Creator → Options → Devices → Android.
